...
public class OkHttp3Utils {
    ...
    //设置缓存目录
    private static File cacheDirectory = new File(BaseApplication.GetInstance().getApplicationContext().getCacheDir().getAbsolutePath(), "wtCache");
...
//                int maxAge = 60 * 60; // 有网络时 设置缓存超时时间1个小时
                int maxAge = 10 * 60; // 有网络时 设置缓存超时时间1个小时
                response.newBuilder()
                        .removeHeader("Pragma")
...
//                int maxStale = 60 * 60 * 24 * 28; // 无网络时，设置超时为4周
                int maxStale = 20 * 60; // 无网络时，设置超时为4周
                response.newBuilder()
                        .removeHeader("Pragma")
