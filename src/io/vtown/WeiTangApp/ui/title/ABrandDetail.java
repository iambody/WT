 ...
 */
public class ABrandDetail extends ATitleBase implements ScrollBottomListener {
	 ...
	 */
	private void IView() {
		brandscroll = (ScrollBottomScrollView) findViewById(R.id.brandscroll);
		brandscroll.setScrollBottomListener(this);
	...
	protected void DataResult(int Code, String Msg, BComment Data) {
		...
		case 0:// 获取全部的信息
			...
			if (StrUtils.JsonContainKey(mJsonObject, "base")) {
				try {
					mBComment.setBase(JSON.parseObject(
							mJsonObject.getString("base"), BDComment.class));
				} catch (JSONException e) {
					e.printStackTrace();
				}
			}
			...
			// try {
			// // Gson gson = new Gson();
			// // java.lang.reflect.Type type = new TypeToken<BComment>()
			// // {}.getType();
			// // mBComment=gson.fromJson(Data.getHttpResultStr(), type);
		...
		case 1:// 获取分类列表数据
			...
			case LOAD_LOADMOREING:
				...
				LateItem = LateItem + mBlComments.size();
	...
	private void ResultData(BShopBrand data) {
		brandscroll.smoothScrollTo(0, 20);
		...
		BDComment bdComment = data.getBase();
	...
	private class BrandLsAp extends BaseAdapter {
		...
		class HorizontalItemClikListener implements OnClickListener {
			...
			public HorizontalItemClikListener(int postion,
					List<BLComment> data, MyBrandItem item, BrandLsAp aBrandLsAp) {
				super();
				myBrandItem = item;
				ap = aBrandLsAp;
				Postion = postion;
	...
	// 点击横向的分类的item进行数据变换s
	private void ChangSort(BLComment d) {
