...
public class AGoodManger extends ATitleBase implements IXListViewListener {
    ...
    public void InItFragment() {
        ...
        fragment_shop_good_manger_brand_horizon_ls.setOnItemClickListener(new OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                for (int i = 0; i < brandAp.getCount(); i++) {
                    brandAp.GetData(i).setSelectBrand(position == i ? true : false);
                }
                brandAp.notifyDataSetChanged();
                BLShopDaiLi myitemdata = (BLShopDaiLi) parent.getItemAtPosition(position);
                // 开始刷新
                CurrentBrandId = myitemdata.getId();
...
//		private List<MyBrandItem> Views= new ArrayList<MyBrandItem>();;
        ...
        public BLShopDaiLi GetData(int i) {
            return datas.get(i);
        }
...
//				Views.add(myItem);
            ...
            myItem.item_fragment_shop_good_manger_brand_name
                    .setBackground(getResources()
                            .getDrawable(
                                    blShopDaiLi.isSelectBrand() ? R.drawable.shape_comment_oval_pre
                                            : R.drawable.shape_comment_oval));
            myItem.item_fragment_shop_good_manger_brand_name
                    .setTextColor(getResources().getColor(
                            blShopDaiLi.isSelectBrand() ? R.color.app_fen
                                    : R.color.grey));
...
//                        if (Sale_Status != 0&&Data.getHttpLoadType()!=LOAD_LOADMOREING)
            ...
            case 55:// 获取品牌的列表
                ...
                if (StrUtils.isEmpty(Data.getHttpResultStr())) {
                    ...
                    datss.add(new BLShopDaiLi("", "全部商品", true));
