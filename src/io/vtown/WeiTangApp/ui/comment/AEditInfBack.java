...
public class AEditInfBack extends ATitleBase implements TextWatcher,
	...
	protected void MyClick(View V) {
		...
		case R.id.comment_infback_submint://
			if (StrUtils
					.isEmpty(comment_infback_ed.getText().toString().trim())) {
				PromptManager.ShowCustomToast(BaseContext, ErrorStr);
				return;
			}
			setResult(
					Tag_Edit,
					new Intent().putExtra(Tag_key, comment_infback_ed.getText()
							.toString().trim()));
			finish();
			break;
