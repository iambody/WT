...
public class MovieRecorderView extends LinearLayout implements OnErrorListener {
    ...
    public MovieRecorderView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mWidth = a.getInteger(R.styleable.MovieRecorderView_recordwidth, 320);// 默认320
        mHeight = a.getInteger(R.styleable.MovieRecorderView_recordheight, 240);// 默认240
...
//			mCamera.lock();
    ...
    private void createRecordDir() {
        ...
        File sampleDir = new File(Environment.getExternalStorageDirectory()
                + File.separator + "aaa/video/");// 录制视频的保存地址
        if (!sampleDir.exists()) {
            sampleDir.mkdirs();
        }
        ...
        // 创建文件
        try {
            mVecordFile = File.createTempFile("re4ngaa", ".mp4", vecordDir);// mp4格式的录制的视频文件
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
...
//		mMediaRecorder.setVideoSource(MediaRecorder.VideoSource.DEFAULT);
        ...
        mMediaRecorder.setVideoEncodingBitRate(1 * 1024 * 1024 * 5);// 设置帧频率，然后就清晰了

        mMediaRecorder.setOrientationHint(90);// 输出旋转90度，保持竖屏录制
        // mMediaRecorder.setOutputFormat(MediaRecorder.OutputFormat.THREE_GPP);
        // !!!!!!!
        // mMediaRecorder.setVideoEncoder(MediaRecorder.VideoEncoder.H264);//(VideoEncoder.MPEG_4_SP);//
        // 视频录制格式
